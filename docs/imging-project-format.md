...
图映开放工程格式用于保存可继续编辑的多图层设计。v2 同时支持图片图层和多语言文字图层，并继续读取 v1 图片工程。文件扩展名为 `.imging`，MIME 类型为 `application/vnd.imging.project+zip`，容器采用标准 ZIP。任何 ZIP 工具都可以解包查看其中的 JSON、原始图片和预览图。
...
图映 v2 写入不压缩（ZIP method 0）的条目，因为 PNG、JPEG、WebP、GIF 和 AVIF 本身已经压缩，这样保存更快且便于流式读取。读取器同时接受 method 0 和 Deflate（method 8）。ZIP64、分卷和加密 ZIP 不属于 v2。

## 清单语义

新工程清单遵循 [`imging-project-v2.schema.json`](imging-project-v2.schema.json)；旧版 [`imging-project-v1.schema.json`](imging-project-v1.schema.json) 仍永久保留。关键约定如下：

- `format` 固定为 `imging-project`。
- 新写入的 `formatVersion` 固定为整数 `2`；读取器接受 `1` 和 `2`。
- 画布单位固定为像素。
...
- v2 文字图层的 `text.value` 保存 UTF-8 原文，`fontId` 保存图映稳定字体标识，`fontFamily` 为第三方读取器提供可读的字体族提示；字号单位为画布像素。
...
图映当前最多读取 24 个素材和 24 个图层、8192 × 8192 的画布、4800 万画布像素以及 512 MB 的工程包。文字内容最多 500 个 Unicode 字符、24 行。读取时会拒绝目录穿越路径、重复路径、加密条目、异常尺寸、未知字体标识和校验失败的素材。
