  ...
  self.addEventListener('message',function(event){
    ...
    (async function(){
      try{
        ...
        var locale=/^(?:en|ja|ko|de|fr)$/.test(data.locale)?data.locale:'zh-CN',result=await self.TYPDFToHTML.convert(bytes,{locale:locale,title:String(data.title||'PDF-固定版式页面'),embedFonts:data.embedFonts!==false,signal:controller.signal,onFirstPage:function(preview){if(!controller.signal.aborted)self.postMessage({type:'preview',id:id,html:preview.html,sourcePages:preview.sourcePages});},onProgress:function(progress){if(!controller.signal.aborted)self.postMessage({type:'progress',id:id,phase:progress.phase,percent:progress.percent,text:progress.text});}});
