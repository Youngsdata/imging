...
- **本地专业去背景**：快速模式自动识别或手选四周任意底色，只移除外框连通区域并反混合去色边；AI 可选约 42 MB 的 ISNet 快速模型、约 219 MB 的 BEN2 专业模型，或约 447 MB、按 2048×2048 处理的 BiRefNet HR-Matting 骨灰级模型。骨灰级面向发丝、薄纱、玻璃和半透明边缘，优先还原原始轮廓；三档都从本站按需下载并独立缓存在浏览器。默认内收 1px、净化背景色边，并支持蒙版柔化与画笔精修，输出 WebP、PNG 或 AVIF。
